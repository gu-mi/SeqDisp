...
#' @title  Plot Precision-Recall curves using ggplot2
#' 
...
#' all considered scearios (panels of plots). This function also output .txt files (for each panel) the precision, recall, 
#' p-value and q-values for each of the models considered (as specified in the \code{model.vec} argument).
#' 
...
#' 
#' @return a list of two component: (1) information for ggplot2 plotting; (2) useful quantities in calculating precision and recall.
...
pl.prec = function(dt = NULL, 
  ...
  for ( i in seq_len(length(noise.vec)) ){
    ...
    if (!is.null(fc.vec)) {
      # sim.data.prepare()
      d.prep = sim.data.prepare(dt = dt, sub.col = sub.col, init.fit = init.fit, 
      ...
      p.info = sim.pwr.plot(sim.data.obj = d.prep,
                            ...
                            title.note = bquote(
                              paste("Simulated Dispersion with ", .(disp.vec[i]), " Trend: ", sigma, " = ", .(noise.vec[i]), sep=""))
      )
      
      m.pwr[[i]] = p.info$pwr
      info.table = p.info$info
      colnames(info.table) = c("model", "recall", "precision", "pvalue", "qvalue", "Fmeasure")
      write.table(x=info.table, quote=FALSE, row.names=FALSE, col.names=TRUE, 
                  file=file.path(out.path, paste0("PR-panel-", panel.label[i] , "-DE", de.vec[i], "-FC", fc.vec[i], ".txt")))
    }
    #
    if (is.null(fc.vec)){
      # sim.data.prepare2()
      d.prep = sim.data.prepare2(dt = dt, sub.col = sub.col, init.fit = init.fit, 
      ...
      colnames(info.table) = c("model", "recall", "precision", "pvalue", "qvalue", "Fmeasure")
      write.table(x=info.table, quote=FALSE, row.names=FALSE, col.names=TRUE, 
                  file=file.path(out.path, paste0("PR-panel-", panel.label[i] , "-DE", de.vec[i], "-Sigma", noise.vec[i], ".txt")))
    }
