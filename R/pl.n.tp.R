...
#' @title  Plot the top N genes vs. True Positve Rate (TPR) plot using ggplot2
#' 
...
pl.n.tp = function(dt = NULL, 
  ...
  for ( i in seq_len(length(noise.vec)) ){
    ...
    if (!is.null(fc.vec)) {
      ...
      colnames(info.table) = c("model", "N", "TP", "pvalue", "qvalue")
      write.table(x=info.table, quote=FALSE, row.names=FALSE, col.names=TRUE, 
                  file=file.path(out.path, paste0("NTP-panel-", panel.label[i] , "-DE", de.vec[i], "-FC", fc.vec[i], ".txt")))
    }
    #
    if (is.null(fc.vec)) {
      ...
      colnames(info.table) = c("model", "N", "TP", "pvalue", "qvalue")
      write.table(x=info.table, quote=FALSE, row.names=FALSE, col.names=TRUE, 
                  file=file.path(out.path, paste0("NTP-panel-", panel.label[i] , "-DE", de.vec[i], "-Sigma", noise.vec[i], ".txt")))
    }
